...
export async function GET() {
  try {
    ...
    // Aggregate by status
    const byStatusAgg = await Request.aggregate<{ label: string; value: number }>([
      { $group: { _id: '$status', count: { $sum: 1 } } },
      { $project: { _id: 0, label: '$_id', value: '$count' } },
    ])

    // Aggregate by service
    const byServiceAgg = await Request.aggregate<{ label: string; value: number }>([
    ...
    const dailyAgg = await Request.aggregate<{ date: string; value: number }>([
